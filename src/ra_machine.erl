...
-type effect() ::
    ...
    %% overloaded effect that includes the machine version to capture
    %% in the snapshot if the snapshot is for a non-current index
    {release_cursor, ra_index(), state(), version()} |
    {aux, term()} |
    garbage_collection.
